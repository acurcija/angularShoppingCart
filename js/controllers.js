...
app.controller("OrderController", function($scope, bagService){
   ...
   $scope.getSubTotal = function() {
       $scope.total = 0;
       bagService.bag.forEach(function(flavors) {
           $scope.total += flavors.quantity * flavors.tea.price;
       });
   ...
   $scope.editItem = function(flavors){
   		$scope.editing = $scope.getOrder.indexOf(flavors);
   }

   $scope.saveField = function(flavors){
    ...
    $scope.removeItem = function(flavors){
        bagService.bag.splice(flavors,1);
   };
